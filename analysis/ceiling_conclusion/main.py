...
figure_outputs = [
    here.parents[2] / "tex" / "ceiling_conclusion.pdf",
]
...
for index, (return_value, choice_intensity, selection_intensity, label) in enumerate(
    ...
    cooperation = [
        hybrid_cooperation(
            return_value, choice_intensity, selection_intensity, intrinsic
        )
...
for index, (return_low, return_high, choice_intensity, label) in enumerate(return_sets):
    cooperation = [
        heterogeneous_return_cooperation(
            return_low, return_high, choice_intensity, high
        )
        for high in counts
    ]
    ax_return.plot(
