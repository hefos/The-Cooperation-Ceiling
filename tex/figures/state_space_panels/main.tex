...
\begin{scope}[shift=(P4)]
    ...
    \pgfmathsetmacro{\cy}{\ph/2 - 0.05}
    \node[vhi] (c0) at (\cx,\cy) {};
    \node[font=\sffamily\scriptsize, text=accent, anchor=north] at (\cx,\cy-0.12) {$\mathbf{a}$};

    \foreach \ang in {0,90,180,270} {
        \pgfmathsetmacro{\nx}{\cx + 1.5*cos(\ang)}
        \pgfmathsetmacro{\ny}{\cy + 1.15*sin(\ang)}
        \node[vtx] (n\ang) at (\nx,\ny) {};
        \draw[->, black!40, line width=0.5pt] (c0) -- (n\ang);
    }
    \node[font=\sffamily\tiny, text=black!60] at (\cx, \cy+0.85) {$\ge \mu/N$};

    \node[note] at (\pw/2, 0.95)
        {every neighbour reachable with mutation};
    \node[note] at (\pw/2, 0.45)
        {$\mu>0 \Rightarrow$ unique steady state};
\end{scope}
