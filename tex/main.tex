...
%However, we can observe that a transition of some sort must occur, and so we
%simply take the probability of not transitioning to any \textbf{different}
%state as $P_{\textbf{a} \to \textbf{a}}$.\\

If we consider mutation within our model, we instead get a transition matrix
...
\begin{large}
    \begin{equation}
        T_\textbf{ab} = 
        \begin{cases}
            (\frac{\sum_{a_i = b_{I(\textbf{ab})}}{f(a_i)}}{N\sum_{a_j}f(a_j)})(1-\sum_{i}\sum_{k}\frac{\mu_{ik}}{N}) + (\frac{\mu_{i,\textbf{b}_I(\textbf{ab})}}{N}) & \text{if }\textbf{b} \in \text{Neb(\textbf{a})}\text{, differing at position }I(\textbf{a,b})\\
            0 & \text{if }\textbf{b} \notin \text{Neb(\textbf{a}) and \textbf{a}}\neq \textbf{b}\\
            1 - \sum_{\textbf{b} \in S \setminus \text{\{\textbf{a}\}}}T_{\textbf{ab}} & \text{if }\textbf{a}=\textbf{b}
        \end{cases}
...
For the case with mutation, we get the following transition matrix:

\begin{large}
    \begin{equation}
        T_\textbf{ab}  = 
        \begin{cases}
            (\frac{1}{N(N-1)}\sum_{a_j=b_{I(\textbf{ab})}}\phi(\pi_{\text{I(\textbf{ab})}}(a) - \pi_{j}(\textbf{a})))(1-\sum_{i}\sum_{k}\frac{\mu_{ik}}{N}) + (\frac{\mu_{i,\textbf{b}_I(\textbf{a,b})}}{N}) & \text{if }\textbf{b} \in \text{Neb(\textbf{a})}\\
            0 & \text{if }\textbf{b} \notin \text{Neb(\textbf{a}) and \textbf{a}}\neq \textbf{b}\\
            1 - \sum_{\textbf{b} \in S \setminus \text{\{\textbf{a}\}}}T_{\textbf{ab}} & \text{if }\textbf{a}=\textbf{b}
        \end{cases}
...
If we consider the case with mutation, our transition matrix becomes the
following:

\[
T_{\textbf{ab}} =
\begin{cases}
(\dfrac{1}{N(m_j - 1)} \, \phi (\Delta \pi_{I(\textbf{ab})}))(1-\sum_{i}\sum_{k}\frac{\mu_{ik}}{N}) + (\frac{\mu_{i,\textbf{b}_I(\textbf{ab})}}{N})
    & \text{if } \textbf{b} \in \mathrm{Neb}(\textbf{a}) \text{ and } j = I(\textbf{a,b}),\\[1.2em]
0
    & \text{if } \textbf{b} \notin \mathrm{Neb}(\textbf{a}) \text{and \textbf{a}} \neq \textbf{b},\\[0.8em]
1 - \sum_{\textbf{b} \in S \setminus \text{\{\textbf{a}\}}}T_{\textbf{ab}} & \text{if }\textbf{a}=\textbf{b}
...
For the case with mutation in the model, our transition matrix instead becomes
the following:

\begin{equation}
T_{\textbf{ab}} =  \begin{cases}
    (\frac{1}{N}\frac{\sum_{a_{j} = b_{I(\textbf{a}, \textbf{b})}}f_j(\textbf{a})}{\sum_{k}f_k(\textbf{a})}\phi(\Delta(f_{I(\textbf{a,b})})))(1-\sum_{i}\sum_{k}\frac{\mu_{ik}}{N}) + (\frac{\mu_{i,\textbf{b}_I(\textbf{a,b})}}{N}) & \text{if \textbf{b}}\in \text{Neb(\textbf{a})}\\
    0 & \text{if \textbf{b}}\notin \text{Neb(\textbf{a}) and \textbf{a}} \neq \textbf{b}\\
    1 - \sum_{\textbf{b} \in S \setminus \text{\{\textbf{a}\}}}T_{\textbf{ab}} & \text{if }\textbf{a}=\textbf{b}
\end{cases}
\end{equation}
