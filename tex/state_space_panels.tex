...
\begin{scope}[shift=(P1)]
    ...
    \foreach \k/\sty/\lab in {1/cnode/$a_1$, 2/dnode/$a_2$, 3/cnode/$a_1$, 4/dnode/$a_2$} {
    ...
    \node[font=\sffamily\scriptsize, text=black!75] at (\pw/2, 0.5)
        {state $\mathbf{a}=(a_1,a_2,a_1,a_2)\in \{a_1,a_2\}^N$};
\end{scope}
...
\begin{scope}[shift=(P2)]
    ...
    \node[ptitle] at (\pw/2, \ph-0.05) {Each state is a vertex of $\{a_1,a_2\}^N$};
    ...
    \node[font=\sffamily\scriptsize, text=accent, anchor=south] at (3.61,2.90) {$\mathbf{b}$};
...
\begin{scope}[shift=(P3)]
    ...
    \foreach \k/\col/\lab in {1/coop/$a_1$,2/defect/$a_2$,3/coop/$a_1$,4/defect/$a_2$} {
    ...
    \foreach \k/\col/\lab in {1/coop/$a_1$,2/coop/$a_1$,3/coop/$a_1$,4/defect/$a_2$} {
...
\begin{scope}[shift=(P4)]
    ...
    \node[note] at (\pw/2, 1.25)
        {all-$a_1$ and all-$a_2$ are absorbing};
