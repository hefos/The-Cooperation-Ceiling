...
parser.add_argument(
    "--p1", type=str, required=True, help="A string value for dynamic one"
)
parser.add_argument(
    "--p2", type=str, required=True, help="A string value for dynamic two"
)
...
merge_conditions = [
    x
    for x in significant_parameter_dict[p1_string]
    if x in significant_parameter_dict[p2_string]
]
...
df = df.drop(
    columns=[
        f"alpha_i_in_{p1_string}",
        f"i_in_{p1_string}",
        f"alpha_i_in_{p2_string}",
        f"i_in_{p2_string}",
    ],
    errors="ignore",
)
...
df["winner_margin"] = np.abs(df[f"p_C_in_{p1_string}"] - df[f"p_C_in_{p2_string}"])
